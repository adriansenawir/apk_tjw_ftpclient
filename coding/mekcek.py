import tkinter as tk
from tkinter import messagebox, filedialog
from ftplib import FTP

# Konfigurasi FTP
ftp_host = '127.0.0.1'
ftp_port = 21
ftp_user = 'hidayat'
ftp_password = 'kelompok9'

class LoginWindow:
    def __init__(self, root):
        self.root = root
        self.root.title('Login')
        self.root.geometry('300x150')

        label_username = tk.Label(root, text='Username')
        label_username.pack()

        self.entry_username = tk.Entry(root)
        self.entry_username.pack()

        label_password = tk.Label(root, text='Password')
        label_password.pack()

        self.entry_password = tk.Entry(root, show='*')
        self.entry_password.pack()

        login_button = tk.Button(root, text='Login', command=self.login)
        login_button.pack()

    def login(self):
        username = self.entry_username.get()
        password = self.entry_password.get()

        if username == ftp_user and password == ftp_password:
            self.root.destroy()
            app_root = tk.Tk()
            FTPClientApp(app_root)
            app_root.mainloop()
        else:
            messagebox.showerror('Login Failed', 'Invalid username or password')

class FTPClientApp:
    def __init__(self, root):
        self.root = root
        self.root.title('Aplikasi FTP')

        label = tk.Label(root, text='Daftar File di Server FTP')
        label.pack()

        self.file_listbox = tk.Listbox(root)
        self.file_listbox.pack()

        upload_button = tk.Button(root, text='Unggah File', command=self.upload_file)
        upload_button.pack()

        download_button = tk.Button(root, text='Unduh File', command=self.download_file)
        download_button.pack()

        delete_button = tk.Button(root, text='Hapus File', command=self.delete_file)
        delete_button.pack()

        refresh_button = tk.Button(root, text='Perbarui Daftar File', command=self.refresh_file_list)
        refresh_button.pack()

        back_button = tk.Button(root, text='Kembali ke Login', command=self.back_to_login)
        back_button.pack()

        self.refresh_file_list()

    def upload_file(self):
        file_path = filedialog.askopenfilename()
        if file_path:
            with open(file_path, 'rb') as file:
                file_name = file_path.split('/')[-1]  # Mengambil nama file dari path
                ftp = FTP(ftp_host)
                ftp.login(user=ftp_user, passwd=ftp_password)
                ftp.storbinary(f'STOR {file_name}', file)  # Menggunakan nama file
                ftp.quit()

    def download_file(self):
        download_dir = filedialog.askdirectory()
        file_name = self.file_listbox.get(self.file_listbox.curselection())
        if file_name and download_dir:
            ftp = FTP(ftp_host)
            ftp.login(user=ftp_user, passwd=ftp_password)
            with open(f"{download_dir}/{file_name}", 'wb') as file:
                ftp.retrbinary(f'RETR {file_name}', file.write)
            ftp.quit()

    def delete_file(self):
        file_name = self.file_listbox.get(self.file_listbox.curselection())
        if file_name:
            ftp = FTP(ftp_host)
            ftp.login(user=ftp_user, passwd=ftp_password)
            ftp.delete(file_name)
            ftp.quit()
            self.refresh_file_list()

    def refresh_file_list(self):
        ftp = FTP(ftp_host)
        ftp.login(user=ftp_user, passwd=ftp_password)
        file_list = ftp.nlst()
        ftp.quit()
        self.file_listbox.delete(0, 'end')
        for file_name in file_list:
            self.file_listbox.insert('end', file_name)

    def back_to_login(self):
        self.root.destroy()
        root = tk.Tk()
        login_window = LoginWindow(root)
        root.mainloop()

def main():
    root = tk.Tk()
    login_window = LoginWindow(root)
    root.mainloop()

if __name__ == '__main__':
    main()
